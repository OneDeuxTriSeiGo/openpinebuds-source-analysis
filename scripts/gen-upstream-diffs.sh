...
#
# Summary: Reads in the upstream sources file and generates diffs
# for each file relative to the current working directory.
#
# Usage: gen-upstream-diffs.sh <upstream-file> <base-commit>
...
sed --regexp-extended --quiet -e "${parse_cmd}" "${upstream_file}" \
    | parallel                                                     \
        --load=90% --jobs=0                                        \
...
echo "[SUCCESS] Upstream Diffs Generated"
